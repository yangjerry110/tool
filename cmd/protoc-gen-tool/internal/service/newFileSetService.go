/*
 * @Author: Jerry.Yang
 * @Date: 2025-03-13 10:42:46
 * @LastEditors: Jerry.Yang
 * @LastEditTime: 2025-03-13 10:45:25
 * @Description: new file set service
 */
package service
...
type NewFileSetService struct {
	FilePath    string
	FileName    string
	AppendFuncs []*config.ProtocHttpRule
}

func (n *NewFileSetService) Generate() error {
	// define filePath + fileName
	parseFilePath := fmt.Sprintf("%s/%s", n.FilePath, n.FileName)
	...
	// Set AppendFunc
	if err := n.setAppendFunc(file); err != nil {
		return err
	}
	return nil
}
 ...
 * @return {*}
 */
func (n *NewFileSetService) setAppendFunc(file *ast.File) error {
	...
	if len(config.ProtocHttpRules) != 0 {
		for _, protoHttpRule := range config.ProtocHttpRules {
			...
			if !isExist {
				n.AppendFuncs = append(n.AppendFuncs, protoHttpRule)
			}
		}
	}
	return nil
