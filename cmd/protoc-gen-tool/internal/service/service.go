...
import (
	...
	"github.com/yangjerry110/tool/cmd/protoc-gen-tool/internal/template/internalTemplate/service"
)
...
func (s *Service) appendService() error {
	...
	newFileSetService := &NewFileSetService{}
	newFileSetService.FilePath = filePath
	newFileSetService.FileName = fileName

	// First, re-render the interface portion of the service based on the protocHttpRules we've obtained
	if err := CreateProtoGenToolService(newFileSetService).Generate(); err != nil {
