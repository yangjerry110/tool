...
import (
	...
	toolgormdb "github.com/yangjerry110/tool/internal/db/gormDb"
...
func (n *NewModel) New() error {
	...
	// Set ModelName
	if err := config.CreateConfig(&config.Model{CliContext: n.CliContext}).SetConfig(); err != nil {
		return err
	}
	...
	// Set ConfigPath
	if err := conf.CreateConf(&conf.Path{ConfigPath: databaseConfPath}).SetConfig(); err != nil {
		return err
	}

	// Set databaseConf
	if err := conf.CreateConf(&toolgormdb.GormDbConfig{}).SetConfig(); err != nil {
		return err
	}
